...
class MoveIt2:
    ...
    def plan(
        ...
        while not future.done():
            # Node của ứng dụng đã được spin bởi executor chính. Gọi
            # rclpy.spin_once() ở đây sẽ tạo executor thứ hai trên cùng action
            # client và có thể làm hỏng wait set của rclpy Humble.
            time.sleep(0.01)
        ...
        try:
            while start_joint_state is None:
                ...
                if self.__joint_state is not None:
                    ...
                    # Callback joint_states do executor chính xử lý.
                    time.sleep(0.01)
    ...
    def wait_until_executed(self) -> bool:
        ...
        while self.__is_motion_requested or self.__is_executing:
            # Không spin node lần hai; xem ghi chú trong plan().
            time.sleep(0.01)
