...
public:
  ...
  void initialize()
  {
    ...
    move_group_interface_->setNumPlanningAttempts(20);  // 设置最大规划尝试次数为 20
    move_group_interface_->setPlanningTime(10.0);         // 设置每次规划的最大时间为 10 秒
	...
	grab_pose.position.z = 0.09;
	grab_pose.position.x = 0.0;		
	...
	{
		attempt_count++;
		std::vector<double> target_joints = {0.0, 0.52, 0.786, 0.20, 0}; // 目标关节角度（单位：弧度）
		// 设置目标关节空间值
